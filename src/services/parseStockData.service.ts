...
export const parseStockData = (apiData: any): IStockDataPoint[] => {
  ...
  if (!rawSeries) {
    throw new ApiError('No se encontró Time Series (Daily) en la respuesta', 502);
  }
...
export const parseSingleStock = (apiData: {
  ...
  if ([openStr, highStr, lowStr, closeStr].some(v => v == null)) {
    throw new ApiError(`Faltan campos de precio para la fecha ${date}`, 502, data);
  }
  ...
  if ([point.open, point.high, point.low, point.close].some(n => isNaN(n))) {
    throw new ApiError(`Datos numéricos inválidos para la fecha ${date}`, 502, point);
  }
